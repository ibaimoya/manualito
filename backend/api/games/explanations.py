"""Explicación cacheada de un juego con invalidación por huella del pool."""
...
async def get_game_explanation(
    ...
    *,
    ...
    """Devuelve la explicación cacheada o la regenera si cambió el pool.

    La huella del pool visible se compara al leer: añadir, borrar, editar o
    re-procesar un manual la cambia, así que la caché se invalida sola sin
    enganches en los flujos de escritura.
    """
    ...
    return await _regenerate_explanation(
        user_id=user_id,
        game_id=game_id,
...
async def _regenerate_explanation(
    *,
    user_id: UUID,
    game_id: UUID,
    fingerprint: str,
    ...
    """Genera las secciones bajo lock para evitar la estampida de LLM."""
...
def _ready_response(explanation: GameExplanation | Row) -> GameExplanationResponse:
    """Convierte la fila cacheada en el contrato público."""
    return GameExplanationResponse(
        status="ready",
