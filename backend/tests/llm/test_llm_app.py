...
def test_prepare_model_on_startup_preloads_model_when_enabled():
    ...
    tags_response.json.return_value = {"models": [{"name": config.OLLAMA_MODEL}]}
    preload_response = MagicMock()
    preload_response.raise_for_status.return_value = None
...
def test_prepare_model_on_startup_logs_warning_when_model_is_missing():
...
def test_prepare_model_on_startup_tolerates_startup_failures():
...
def test_prepare_model_on_startup_tolerates_startup_timeout():
    """Si el contexto temporal corta la comprobación inicial, el servicio sigue."""
    client = AsyncMock()
...
def test_generate_returns_504_when_asyncio_timeout_expires(client, override_http_client):
    """El timeout estructurado de asyncio se traduce al mismo error público."""
    override_http_client.post.side_effect = TimeoutError("slow")
