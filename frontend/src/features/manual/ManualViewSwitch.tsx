...
import { Columns2, FileText, Image } from 'lucide-react';
...
const options = [
  ...
  { value: 'original', icon: Image },
  { value: 'compare', icon: Columns2 },
] as const;
...
function ViewOptions({
  ...
  return (
    ...
    >
      {options
        .filter((option) => wide || option.value !== 'compare')
        .map(({ value: option, icon: Icon }) => (
          <RadioGroup.Item
            key={option}
            value={option}
            className={cn(
              'relative inline-flex min-h-10 items-center justify-center gap-2 rounded-[4px] px-2 text-[13px] font-medium text-fg-2 hover:bg-fg/[0.035] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary data-[state=checked]:text-fg @2xl/app:px-3',
            )}
          >
            <Icon size={16} aria-hidden="true" className="hidden @2xl/app:block" />
            {t(`workspace.${option}`)}
            {option === value && (
              <motion.span
                aria-hidden="true"
