...
import { SavedGameSearch } from '@/features/manual/SavedGameSearch';
...
function HistoryScreen() {
  const { data, isPending, isError } = useQuery(manualsQueryOptions());
  ...
  if (isPending) {
    ...
    content = (
      <ul className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))' }}>
        {manuals.map((m) => (
          <li key={m.id} className="group">
            <ManualListItem manual={m} onDelete={() => del.mutate(m.id)} />
          </li>
        ))}
      </ul>
    );
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-col gap-4 px-5 pb-10 pt-4 md:max-w-5xl md:px-8 md:pt-10">
      <header className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <h1 className="font-display text-2xl font-bold tracking-tight md:text-3xl">Tus juegos</h1>
        {manuals.length > 0 ? <SavedGameSearch manuals={manuals} /> : null}
      </header>
    ...
    <div className="overflow-hidden rounded-2xl border border-border bg-bg">
      <div className="flex items-stretch gap-2 pr-2">
        <div className="flex-1">
          <ManualCard
            manual={manual}
            meta={formatDate(manual.created_at)}
      ...
      {confirming ? (
        <div className="flex items-center justify-end gap-2 border-t border-border bg-error-bg p-3">
          <span className="mr-auto text-sm text-error">¿Borrar {name}?</span>
