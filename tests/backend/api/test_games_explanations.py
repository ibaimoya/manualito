...
def test_explanation_endpoint_without_pool_returns_stable_404(
    client,
    monkeypatch,
    override_auth_and_db,
):
    ...
    assert any(
        error["code"] == "manual_context_not_found"
        for error in response.json()["errors"]
    )


def test_explanation_cache_hit_skips_generation(monkeypatch):
    """Con la huella intacta se sirve la caché sin tocar el LLM."""
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(return_value=_cached_explanation()),
    )
...
def test_explanation_lock_winner_reuses_fresh_cache(monkeypatch):
    ...
    monkeypatch.setattr(
        explanations_module.repository,
        "get_game_explanation",
        AsyncMock(side_effect=[None, _cached_explanation()]),
    )
    monkeypatch.setattr(explanations_module, "generate_game_answer", generate_mock)
...
def test_explanation_without_visible_manuals_raises_context_404(monkeypatch):
    """Un pool vacío no genera nada y responde contexto no encontrado."""
    monkeypatch.setattr(
        explanations_module.repository,
        "get_game_for_detail",
        AsyncMock(return_value=SimpleNamespace(id=_GAME_ID)),
...
def test_get_pool_fingerprint_returns_none_for_empty_pool():
...
def test_get_game_explanation_repository_filters_by_user_and_game():
    """La caché de explicaciones se busca por usuario y juego concretos."""
    cached = object()
    ...
    compiled = _compile(session.statement)
    assert "game_explanations.user_id =" in compiled
    assert "game_explanations.game_id =" in compiled


def test_upsert_game_explanation_compiles_to_atomic_on_conflict_update():
    """El upsert por (usuario, juego) reemplaza secciones y huella de una vez."""
    row = SimpleNamespace(
        sections=_SECTIONS,
...
def _patch_lock(monkeypatch, lock_session) -> None:
